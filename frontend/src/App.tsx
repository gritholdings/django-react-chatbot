...
export default function App() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/chat_models/').then(response => {
      setModelList(response.data.models.map((model: string[]) => ({ name: model[0], value: model[1] })));
  ...
  const modelListOptions = () => {
    return modelList.map(model => model.value);
  }
  ...
  return (
    <div className="app bg-stone-700 w-full h-screen flex flex-col place-items-center">
      <div className="app-body w-96">
        ...
        <Textarea ref={textareaRef} placeholder="Message Chatbot" onChange={handleMessageChange} onSubmit={handleSubmit}/>
      </div>
    </div>
  );
}